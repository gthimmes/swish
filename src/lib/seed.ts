...
export async function seedDatabase(prisma: PrismaClient) {
  ...
  await makeItem({
    ...
    labels: ["frontend"],
    rank: 1020,
    description: "Select many backlog rows and restage, reassign, set priority, or delete in one move.",
    spec: {
      status: "APPROVED",
      problem: "Editing items one at a time is slow when triaging a big backlog.",
      goals: "Multi-select rows (incl. select-all) and apply stage/assignee/priority or delete in bulk.",
      nonGoals: "Bulk label editing and multi-select on the board itself (backlog only for v1).",
      approach: "Selection state on the backlog; a floating bulk bar posts to a /api/items/bulk updateMany/deleteMany.",
      criteria: [
        { text: "Select-all and per-row selection with a bulk bar", done: true },
        { text: "Bulk set stage / assignee / priority", done: true },
        { text: "Bulk delete with confirm", done: true },
      ],
      tests: [{ text: "E2E: select-all, bulk priority, bulk reassign, bulk delete, clear", status: "PASS" }],
    },
  });
  await makeItem({
