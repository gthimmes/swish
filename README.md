...
- **Task dependencies** — directional blocks / blocked-by, with a Blocked chip on cards and blocked outlines on the Timeline.
- **Custom fields** — per-project typed fields (text/number/select/URL), edited on each item's detail panel.
- **Cycles / sprints** — optional time-boxed iterations with per-cycle progress and Upcoming/Active/Completed status; assign items from their detail panel. Methodology stays your choice.
